 ...
 *
 * @author Remy Maucherat, Adam Hawkins
...
public class CometConnectionManagerValve extends ValveBase
     ...
     */
    protected static String COMET_REQUESTS_ATTRIBUTE = "org.apache.tomcat.comet.connectionList";
    ...
    public void invoke(Request request, Response response)
        ...
        if (request.isComet() && !response.isClosed()) {
            ...
            synchronized (session) {
                Request[] requests = getConnectionList(session);
                if (requests == null) {
                    requests = new Request[1];
                    requests[0] = request;
                    setConnectionList(session, requests);
                } else {
                    Request[] newRequests = 
                    ...
                    setConnectionList(session, newRequests);
                }
            }
        }
            ...
            if (!ok || response.isClosed() 
                ...
                if (session != null) {
                    synchronized (session) {
                        Request[] reqs = null;
                        try {
                             reqs = getConnectionList(session);
                        ...
                        if (reqs != null) {
                            ...
                            if (found) {
                                if (reqs.length > 1) {
                                    ...
                                    try {
                                    	setConnectionList(session, newConnectionInfos);
                                    } catch (IllegalStateException ise) {
                                        // Ignore - session has been invalidated
                                        // Listener will have cleaned up
                                    }
    ...
    public void sessionDestroyed(HttpSessionEvent se) {
        ...
        Request[] reqs = getConnectionList(se.getSession());
        if (reqs != null) {
            for (int i = 0; i < reqs.length; i++) {
                Request req = reqs[i];
                try {
    ...
    public  Request[] getConnectionList(HttpSession s){
		Object o = s.getAttribute(COMET_REQUESTS_ATTRIBUTE);
		if(o == null) return null;
		return ((ConnectionList)o).get();
    }
    
    public void removeConnectionList(HttpSession s){
    	s.removeAttribute(COMET_REQUESTS_ATTRIBUTE);
    }
    
    public void setConnectionList(HttpSession s, Request[] r){
    	s.setAttribute(COMET_REQUESTS_ATTRIBUTE, new ConnectionList(r));
    }
